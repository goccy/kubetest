...
func _main(args []string, opt option) error {
	...
	if timeout != 0 {
		...
		defer func() {
			fmt.Fprintf(os.Stderr, "kubetest-agent: timeout (%s)\n", opt.Timeout)
			cancel()
		}()
...
func main() {
	...
	if err != nil {
		...
		if !ok {
			fmt.Fprintf(os.Stderr, "kubetest-agent: unknown parsed option error: %T %v\n", err, err)
			os.Exit(exitFailure)
		}
	...
	if err := _main(args, opt); err != nil {
		fmt.Fprintf(os.Stderr, "kubetest-agent: %+v\n", err)
		os.Exit(exitFailure)
	}
	os.Exit(exitSuccess)
}
